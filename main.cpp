...
bool SHC(int wierzcholek,vector<vector<int>>& mac,int n,int start)
{
    ...
    if (znaleziona_sciezka.size() == n) {
        vector<int> sasiedzi_ostatniego = sasiedzi(wierzcholek, mac, n);
    ...
    vector<int> s = sasiedzi(wierzcholek, mac, n);
...
int main() {
    ...
    if (test) {
        ...
        if (r == 1) { //euler wypisuje od tylu
            for (int i = znaleziona_sciezka.size() - 1; i >= 0; i--) {
                cout << znaleziona_sciezka[i] + 1 << " ";
            }
