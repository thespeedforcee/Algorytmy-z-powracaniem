...
struct MacierzGrafu {
    ...
    void zbuduj(const vector<vector<int>>& mat_sasiedztwa, bool skierowany) {
        for (int i = 1; i <= n; i++) {
            ...
            for (int j = 1; j <= n; j++) {
                bool krawedz_w_przod = (mat_sasiedztwa[i][j] != 0);
                bool krawedz_w_tyl = (mat_sasiedztwa[j][i] != 0);
...
bool czySilnieSpojny(const vector<vector<int>>& mat, int n, const vector<int>& out_deg, const vector<int>& in_deg) {
    ...
    while (!q.empty()) {
        int v = q.front(); q.pop();
        for (int u = 1; u <= n; u++) {
            if (mat[u][v] != 0 && !odwiedzone[u]) { // odwrocony kierunek
                odwiedzone[u] = true; q.push(u);
            }
        }
    }
...
vector<int> sasiedzi(int wierzcholek,vector<vector<int>>&mac,int n,int rodzaj) //czyli nastepnicy, do SHC
{
    vector<int> s;
    if (rodzaj == 1) { // Macierz sasiedztwa
        for (int i = 1; i <= n; i++) {
            if (mac[wierzcholek][i] != 0) {
                s.push_back(i);
            }
        }
    } else { // Macierz grafu
...
bool SEC(int wierzcholek,vector<vector<int>>& mac,int n,int m,bool czyskier,int odwiedzone_kr,int start) //m - liczba krawedzi grafu, EULER
{
    ...
    for(int i=1;i<=n;i++)
    {
        if(mac[wierzcholek][i]!=0)
        {
            int zapamietana = mac[wierzcholek][i];
            mac[wierzcholek][i] = 0; //odcinam sciezke zeby przypadkiem nie wrocic
            int zapamietana_tyl = 0; //zapamietuje krawedz powrotna

            if(!czyskier){
                zapamietana_tyl = mac[i][wierzcholek];
                mac[i][wierzcholek]=0;}
            ...
            licznik_backtrackow++;
            mac[wierzcholek][i] = zapamietana; //wracamm do zapamietanej i probuje z innego miejsca
            if(!czyskier)
            {
                mac[i][wierzcholek] = zapamietana_tyl;
            }
        }
    }
    return false;
}
bool SHC(int wierzcholek,vector<vector<int>>& mac,int n,int start,int rodzaj)
{
    odwiedzone[wierzcholek] = true; 
    znaleziona_sciezka.push_back(wierzcholek); //dodaje wierzcholek do wektora trasy

    // czy zwiedzilam wszystkie wierzcholki i moge wrocic do poczatku
    if (znaleziona_sciezka.size() == n) {
        vector<int> sasiedzi_ostatniego = sasiedzi(wierzcholek, mac, n,rodzaj);
    ...
    //jak nie znalazl cyklu to szuka dalej drogi
    vector<int> s = sasiedzi(wierzcholek, mac, n,rodzaj);
    
    for (int i = 0; i < s.size(); i++) {
        int sasiad = s[i];
        
        // idzie tylko do nieodwiedzonych
        if (!odwiedzone[sasiad]) { 
            
            if (SHC(sasiad, mac, n, start,rodzaj)) {
                return true; 
            }
        }
    }
    
    // jesli weszlam do miasta z ktorego nie ma wyjscia lub sasiedzi odwiedzeni
    licznik_backtrackow++;
    odwiedzone[wierzcholek] = false;
    znaleziona_sciezka.pop_back();   // usuwa wierzcholek z wektora trasy
...
int main() {
    ...
    cout << "1. Cykl Eulera (SEC)" << endl;
    cout << "2. Cykl Hamiltona (SHC)" << endl;
    cout << "Wybor: ";
    r = wczytajLiczbe(1, 2);

    cout << "\n--- WERSJA PROBLEMU ---" << endl;
    cout << "1. Wersja Decyzyjna (DEC / DHC)" << endl;
    cout << "2. Wersja Przeszukiwania (SEC / SHC)" << endl;
    cout << "Wybor: ";
    ...
    if (wersja == 1) {
        cout << "Uruchamiam algorytm decyzyjny (Twierdzenia)" << endl;
    }else {
        cout << "Uruchamiam przeszukiwanie z powrotami" << endl;
    }
    ...
    }else if(wersja == 2)
    {
        if (r == 1) {
            ...
            test = SHC(1, macierz, n, 1,rodzaj_dla_generatora);
        }
    }
    ...
    if (test) {
        ...
        if (r == 1) { //euler wypisuje od tylu przez rekurencje
            for (int i = znaleziona_sciezka.size() - 1; i >= 0; i--) {
                cout << znaleziona_sciezka[i] << " ";
            }
        } else {//hamilton normalnie
